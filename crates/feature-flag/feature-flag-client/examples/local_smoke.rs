...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = FeatureFlagClient::connect_with(
        "http://localhost:50051",
        "feature-flag-client-example",
    ...
    let beta = client
        .resolve_bool(
            "new-checkout",
            Context::new("u1").string("email", "a@anurag.sh"),
        )
        .await?;
    println!(
    ...
    let other = client
        .resolve_bool(
            "new-checkout",
            Context::new("u2").string("email", "a@other.com"),
        )
        .await?;
    println!(
    ...
    let missing = client
        .resolve_bool("does-not-exist", Context::new("u3"))
        .await?;
