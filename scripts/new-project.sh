...
build_resources() {
  ...
  $with_secrets && printf '  - secret.yaml\n  - secretstore.yaml\n'
  return 0
}
...
generate_files() {
  ...
  local app_tmpl="application.yaml.tmpl"
  $is_public && app_tmpl="application.public.yaml.tmpl"

  render "$tmpl_dir/$app_tmpl"            >"$dir/application.yaml"
  ...
  if $with_secrets; then
    render "$tmpl_dir/secretstore.yaml.tmpl" >"$dir/manifests/secretstore.yaml"
    render "$tmpl_dir/secret.yaml.tmpl"      >"$dir/manifests/secret.yaml"
  fi
